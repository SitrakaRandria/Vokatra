...
from app.utils.validators import validate_positive_decimal, validate_non_negative_decimal
...
class Listing(Base):
    ...
    def validate_available_quantity(self, key: str, value: Decimal) -> Decimal:
        """Valide que la quantité disponible est non négative et ne dépasse pas la quantité totale."""
        if not validate_non_negative_decimal(value):
            raise ValueError(f"{key} doit être positif ou nul: {value}")
        if hasattr(self, 'total_quantity') and self.total_quantity is not None and value > self.total_quantity:
